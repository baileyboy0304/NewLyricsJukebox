...
- **Primary — Shazam** (`shazam.py`, ShazamIO): audio converted to WAV via
  stdlib `wave` + numpy (no ffmpeg); optional scipy resample (warns and falls
  back to `numpy.interp` if scipy missing). Rejects matches when
  `|timeskew|` or `|freqskew| > 0.08`.
...
- **Optional — Local fingerprint** (`local.py` + `daemon.py`): SoundFingerprinting
  `sfp-cli` daemon for sub-100 ms offline queries against a user FLAC DB; gated
  by `LOCAL_FP_ENABLED`; subprocess fallback intentionally disabled.
...
- **Persistence:** per-song JSON in the lyrics DB
  (`{artist} - {title}.json`) with `saved_lyrics` (line-sync per provider),
  `word_synced_lyrics` (per provider), `preferred_provider`,
  `preferred_word_sync_provider`, `is_instrumental_manual`, `word_sync_offset`.
...
> **Note for the rebuild:** the §7 "minimal viable core" above was the original
> conservative proposal. The actual rebuild scope (see the rebuild brief and
> `CLAUDE.md`) intentionally differs on three points: **multi-player demux is
> retained** (the player-select UI needs it), **both cloud recognizers are kept**
> (Shazam + ACRCloud; local FP permanently excluded), and **all lyrics providers
> are kept**. Treat the brief/`CLAUDE.md` as authoritative where they diverge.
